...
class TestBurger:
    ...
    def test_remove_ingredient(self, mock_ingredient_sauce,
        ...
        assert (len(burger.ingredients) == 1
                and burger.ingredients[0] == mock_ingredient_filling)

    def test_move_ingredient(self, mock_ingredient_sauce,
                             mock_ingredient_filling):
        burger = Burger()
        burger.add_ingredient(mock_ingredient_sauce)
        burger.add_ingredient(mock_ingredient_filling)
        ...
        assert (burger.ingredients[0] == mock_ingredient_filling
                and burger.ingredients[1] == mock_ingredient_sauce)
    ...
    def test_get_price_parametrized(self, mock_bun, sauce_price,
                                    filling_price, expected_total):
        burger = Burger()
        burger.set_buns(mock_bun)  # mock_bun.price = 100.0
        ingredient1 = Ingredient(INGREDIENT_TYPE_SAUCE,
                                 "sauce1", sauce_price)
        ingredient2 = Ingredient(INGREDIENT_TYPE_FILLING,
                                 "filling1", filling_price)
        burger.add_ingredient(ingredient1)
        burger.add_ingredient(ingredient2)
        assert burger.get_price() == expected_total

    def test_get_receipt(self, mock_bun, mock_ingredient_sauce,
                         mock_ingredient_filling):
        burger = Burger()
        burger.set_buns(mock_bun)
        burger.add_ingredient(mock_ingredient_sauce)
